...
public class kNN_GA {
    ...
    public static double[][] train = new double [TRAIN_SIZE][FEATURE_SIZE]; //data to train 
     ...
     public static int[] val_label=new int[VAL_SIZE]; //actual target/class label for validation data nei
    ...
    for(int j=0; j<POP_SIZE; j++) 
    {
      for (int k=0; k<FEATURE_SIZE; k++) {
        if(rn.nextDouble()<0.5) //how many times, pcrossover=0.5
        {
          ...
          cross_array[k]=new_sol[rnda][k];
          new_sol[rnda][k]=new_sol[rndb][k];
          new_sol[rndb][k]=cross_array[k];
        }
  ...
  public static double KNN(double[][] train, double[][] val, int[] train_label, int[] val_label, boolean[] sol) {
    ...
    int NUM_NEIGHBOUR = 14; //replace with optimal k value
